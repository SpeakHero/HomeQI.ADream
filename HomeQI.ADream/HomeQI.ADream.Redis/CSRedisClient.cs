...
namespace CSRedis
{
    public partial class CSRedisClient
    {
        ...
        private DateTimeOffset dt1970 = new DateTime(1970, 1, 1);
        /// <summary>
        /// 缓存壳
        /// </summary>
        ...
        public T CacheShell<T>(string key, string field, int timeoutSeconds, Func<T> getData, Func<(T, long), string> serialize, Func<string, (T, long)> deserialize)
        {
            ...
            if (cacheValue != null)
            {
                try
                {
                    ...
                    if (DateTimeOffset.Now.Subtract(dt1970.AddSeconds(value.Item2)).TotalSeconds <= timeoutSeconds) return value.Item1;
                }
                catch
                {
            ...
            var ret = getData();
            HashSet(key, field, serialize((ret, (long)DateTimeOffset.Now.Subtract(dt1970).TotalSeconds)));
            return ret;
        }
        /// <summary>
        ...
        public T[] CacheShell<T>(string key, string[] fields, int timeoutSeconds, Func<string[], (string, T)[]> getData, Func<(T, long), string> serialize, Func<string, (T, long)> deserialize)
        {
            ...
            for (var a = 0; a < ret.Length; a++)
            {
                if (cacheValue[a] != null)
                {
                    try
                    {
                        ...
                        if (DateTimeOffset.Now.Subtract(dt1970.AddSeconds(value.Item2)).TotalSeconds <= timeoutSeconds)
                        {
                            ret[a] = value.Item1;
                            continue;
                        }
            ...
            if (fieldsMGet.Any())
            {
                ...
                foreach (var d in data)
                {
                    ...
                    mset[msetIndex++] = serialize((d.Item2, (long)DateTimeOffset.Now.Subtract(dt1970).TotalSeconds));
                ...
                foreach (var fieldNull in fieldsMGet.Keys)
                {
                    ...
                    mset[msetIndex++] = serialize((default(T), (long)DateTimeOffset.Now.Subtract(dt1970).TotalSeconds));
                }
...
return 0", $"CSRedisPSubscribe{subscrKey}", "", trylong.ToString());
        ...
        /// <returns></returns>
        public string ChangDb(int index)
        {
            if (_clusterRule == null || ClusterNodes.Count > 1)
            {
                var pool = ClusterNodes.First().Value;
                using (var conn = pool.GetConnection())
                {
                    return conn.Client.Select(index);
                }
            }
            return string.Empty;
        }
        public async Task<string> ChangDbAsync(int index)
        {
            if (_clusterRule == null || ClusterNodes.Count > 1)
            {
                var pool = ClusterNodes.First().Value;
                using (var conn = pool.GetConnection())
                {
                    return await conn.Client.SelectAsync(index);
                }
            }
            return string.Empty;
        }
